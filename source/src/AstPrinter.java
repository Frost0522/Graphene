...
public class AstPrinter implements AstVisitor {
    ...
    private String formatStr(String string) {
        ...
        for (String str : strArray) {
            for (int i = 0; i < depth; i++) {str = "   "+str;} builder.append(str+"\n");
        } return builder.toString();
    }
    private String formatStr(String string, Boolean newLine) {
        if (!formatSwitch) {formatSwitch = true; return string;} if (newLine) {formatStr(string);}
        StringBuilder builder = new StringBuilder(); String[] strArray = string.split("\n");
        for (String str : strArray) {
            for (int i = 0; i < depth; i++) {str = "   "+str;} builder.append(str+"\n");
        } String subStr = builder.toString().substring(0,builder.toString().length()-1); return subStr;
    }

    public String toString() {return astStr.toString().trim();}

    @Override
    public void visit(IdNode idNode) {
        if (depth==0) {depth=2;} astStr.append(formatStr(idNode.toString(),false));
    }

    @Override
    public void visit(LitNode litNode) {
        if (depth==0) {depth=2;} astStr.append(formatStr(litNode.toString(),false));
    }

    @Override
    public void visit(BinaryNode binNode) throws Analyzer {
        if (depth < 2) {depth+=2; astStr.append(formatStr("operator "+binNode.getSymbol()));
        } else {astStr.append(formatStr("operator ",false)+binNode.getSymbol()+"\n");} depth++;
        if (binNode.getLeft() != null) {
            astStr.append(formatStr("left ",false));
            formatSwitch = false; binNode.getLeft().accept(this); astStr.append("\n");
        }
        if (binNode.getRight() != null) {
            astStr.append(formatStr("right ",false));
            formatSwitch = false; binNode.getRight().accept(this);
        } depth--;
    }
    ...
    public void visit(NotNode notNode) throws Analyzer {
        ...
        depth++; notNode.getNode().accept(this); depth--;    
    }
    ...
    public void visit(CallNode callNode) throws Analyzer {
        ...
        for (Node arg : callNode.getArgs()) {
            arg.accept(this); if (arg!=callNode.getArgs().getLast()) {astStr.append("\n");}
        } depth-=2;
    }
    ...
    public void visit(FnNode fnNode) throws Analyzer {
        ...
        for (Node bodyNode : fnNode.getBodyNodes()) {bodyNode.accept(this); astStr.append("\n");}
    }

    @Override
    public void visit(PrgrmNode prgrmNode) throws Analyzer {
        for (Node fnNode : prgrmNode.getFunctions()) {fnNode.accept(this); astStr.append("\n");}
    }
