...
class State_1 extends State {
    ...
    Token process(ArrayList<State> stateLst, ArrayList<Integer> charLst, Integer currentChar) throws Analyzer, IOException {
        if (isAlpha(currentChar) || isNumeric(currentChar) || currentChar.equals(Lexical.UNDERSCORE.value())) {
            ...
            Scanner_v2.next();
            for (Entry<Lexical, ArrayList<Integer>> entry : Scanner_v2.lexicalMap.entrySet()) {
                if (!(entry.getValue()).equals(charLst)) continue;
                switch (entry.getKey()) {
                    case INTEGER: {
                        return new Token(Lexical.INTEGER, charLst);
                    }
                    case BOOLEAN: {
                        return new Token(Lexical.BOOLEAN, charLst);
                    }
                    case IF: {
                        return new Token(Lexical.IF, charLst);
                    }
                    case ELSE: {
                        return new Token(Lexical.ELSE, charLst);
                    }
                    case NOT: {
                        return new Token(Lexical.NOT, charLst);
                    }
                    case AND: {
                        return new Token(Lexical.AND, charLst);
                    }
                    case FN: {
                        return new Token(Lexical.FN, charLst);
                    }
                    case OR: {
                        return new Token(Lexical.OR, charLst);
                    }
                    case TRUE: {
                        return new Token(Lexical.BOOLEAN_LITERAL, charLst);
                    }
                    case FALSE: {
                        return new Token(Lexical.BOOLEAN_LITERAL, charLst);
                    }
